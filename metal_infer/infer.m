    ...
    @autoreleasepool {
        ...
        if (!prompt_tokens_path) {
            snprintf(default_prompt_tokens, sizeof(default_prompt_tokens),
                     "metal_infer/tokenizer.bin");
            if (access(default_prompt_tokens, R_OK) != 0) {
                snprintf(default_prompt_tokens, sizeof(default_prompt_tokens),
                         "tokenizer.bin");
            }
        ...
        printf("Config:   %s/config.json (layers=%d, experts=%d, hidden=%d, moe=%d, shared=%d)\n", cfg.model_path, cfg.num_layers, cfg.num_experts, cfg.hidden_dim, cfg.moe_intermediate, cfg.shared_intermediate);
        printf("Model:    %s\n", model_path);
        printf("Weights:  %s\n", weights_path);
        printf("Manifest: %s\n", manifest_path);
        printf("Vocab:    %s\n", vocab_path);
        printf("Tokenizer:%s%s\n", prompt_tokens_path ? " " : " (none)", prompt_tokens_path ? prompt_tokens_path : "");
